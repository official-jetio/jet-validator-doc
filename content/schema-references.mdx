...
4. Replaces `$ref` with function call: `*validate_definitions_user_abc123`
   **Or:**
5. Compile the referenced schema
6. Directly inline the generated code to avoid function calls
...
4. All functions compiled once, reused for validation
   **\*Or**
5. Finds `$ref: "#/definitions/order"` → attempt to inline it.
6. Inside order, finds `$ref: "#/definitions/customer"` → attempts to inline it inside `$ref: "#/definitions/order"`
7. Inside order, finds `$ref: "#/definitions/item"` → attempts to inline it inside `$ref: "#/definitions/order"`
8. If all nested $refs of "#/definitions/order" can be inlined then "#/definitions/order" it self can also be inlined, avoiding function call overhead
...
Due to our resolution process, @jetio/validator never has infinite recursion problems, unlike other validators which exceed the maximum stack depth with certain schemas. @jetio/validator's recursion only goes as deep as the data structure itself, so there is no **INFINITE RECURSION** problem in @jetio/validator.
