...
interface ValidatorOptions {
  ...
  removeAdditional?: boolean | "all"; // Remove additional properties
  ...
  verbose?: boolean | "value" | "path"; // Verbose error messages
...
interface CodeContext {
  ...
  schemaPath: string; // Path in schema
  fullAccess: ReadonlyArray<AccessSegment> // full access pattern from root with their type 
  rootDataVar: string; // 'data' if in ref, 'rootData' if in main function
  errorVariable: string; 
  allErrors: boolean; // Whether allErrors is enabled
  functionName: string; // Name of the validation function (changes if inside a $ref function)
  // validation utilities
  resolveDataPointer: (pointer: string) => string; // resolve $data pointers
  buildError(error: KeywordValidationError): string; // builds error and inject into code
  addEvaluatedProperty(prop: string): string; // track unevaluatedItems
  addEvaluatedItem(item: number): string; // track unevaluatedProperties
}
```
  ...
  code: (schemaValue, parentSchema, context) => {
    ...
    return `
      if (${context.dataVar} % 2 !== 0) {
        ${context.errorVariable} ??= []; // Error variable is null if no error has been collected
        ${context.errorVariable}.push({
...
- Must handle `allErrors` mode by checking `context.allErrors` so you know when to return on error or just collect and continue
- `context.errorVariable` is available whether `allErrors` is `true` or not, the difference `allErrors` mode is that you don't return on error.
