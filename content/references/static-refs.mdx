...
JetValidator returns the complete path to a referenced schema in errors — from the origin all the way to the failure point when `verbose` is set to `true` or `'path'`:
